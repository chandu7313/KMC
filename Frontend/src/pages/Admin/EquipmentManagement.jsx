...
import { Plus, Edit2, Trash2, Search, Upload, Settings, ShoppingCart, XCircle, DollarSign, Database, Tag, Info } from 'lucide-react';
...
const EquipmentManagement = () => {
    ...
    const fetchEquipments = async () => {
        try {
            ...
            const { data } = await axios.get(`${backendUrl}/api/equipment/admin-list`);
            if (data.success) {
    ...
    const fetchOrders = async () => {
        try {
            axios.defaults.withCredentials = true;
            const { data } = await axios.get(`${backendUrl}/api/equipment/admin-orders`);
            if (data.success) {
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            let res;
            if (editMode) {
                res = await axios.put(`${backendUrl}/api/equipment/update/${selectedId}`, fd);
            } else {
                res = await axios.post(`${backendUrl}/api/equipment/add`, fd);
            }
    ...
    const handleDelete = async (id) => {
        ...
        try {
            axios.defaults.withCredentials = true;
            const { data } = await axios.delete(`${backendUrl}/api/equipment/delete/${id}`);
            if (data.success) {
    ...
    const handleStatusUpdate = async (orderId, newStatus) => {
        try {
            axios.defaults.withCredentials = true;
            const { data } = await axios.post(`${backendUrl}/api/equipment/update-status`, { orderId, status: newStatus });
            if (data.success) {
                toast.success(data.message);
                fetchOrders();
            }
    ...
    const resetForm = () => {
        setFormData({ name: '', description: '', price: '', category: '', stock: '', specifications: '' });
        setImage(false);
        setEditMode(false);
        setSelectedId(null);
    };
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-slate-500 text-sm italic">Machinery and Tool Inventory</p>
                </div>
                {activeTab === 'inventory' && (
                    <button 
            ...
            {activeTab === 'inventory' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div key={item._id} className="bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-xl transition-all group">
                            ...
                            <div className="p-5 space-y-3">
                                ...
                                <div className="flex items-center gap-2 text-xs text-slate-500">
                                    <Database size={12} />
                                    <span>Stock: <span className={`font-bold ${item.stock < 5 ? 'text-rose-500' : 'text-slate-700'}`}>{item.stock}</span> units</span>
                                </div>
                    ...
                    {equipments.length === 0 && <div className="col-span-full py-20 text-center text-slate-300">No equipments found.</div>}
                </div>
            ) : (
                <div className="bg-white rounded-[32px] shadow-sm border border-slate-100 overflow-hidden">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="text-xs space-y-0.5">
                                            {order.items.map((it, idx) => (
                                                <div key={idx}>{it.equipmentId?.name} x {it.quantity}</div>
                                            ))}
                                        </div>
                                    </td>
                    ...
                    {orders.length === 0 && <div className="p-20 text-center text-slate-300">No equipment orders yet.</div>}
                </div>
            )}
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-[32px] w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in duration-300">
                        <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                            <h2 className="text-xl font-bold text-slate-900">{editMode ? 'Edit Equipment' : 'Add Equipment'}</h2>
