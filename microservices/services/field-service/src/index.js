...
const app = express();
const logger = createLogger('field-service');
...
app.get('/health', (req, res) => res.status(200).json({ status: 'healthy', service: 'field-service', uptime: process.uptime() }));

// ── Prometheus Metrics ──
app.get('/metrics', metrics.metricsRoute);

// TODO: Add field management routes
app.use((req, res) => res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: `${req.method} ${req.path} not found` } }));
app.use(errorHandler);

const server = app.listen(PORT, '0.0.0.0', () => logger.info(`Field service running on 0.0.0.0:${PORT}`));
